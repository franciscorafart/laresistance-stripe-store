...
const SplitForm = (props) => {
  ...
  const [formValidState, setFormValidState] = useState(initialFormState);
    ...
    const saveCompletedTransaction = token => {
        fetch('/transaction_complete', {
            method: 'POST',
            cache: 'no-cache',
            headers: {
        ...
        .then((data) => {
            ...
            stripe.confirmCardPayment(data.clientSecret, {
                ...
                if (result.paymentIntent) {
                    props.handleClose();
                    props.displayAlert(true, 'success', 'Your payment was submitted successfully. Reach out to email@gmail..com if you have any questions');
                    props.handleShowDownload(token);
                    saveCompletedTransaction(token);
                }
            });
        })
        .catch((error) => {
